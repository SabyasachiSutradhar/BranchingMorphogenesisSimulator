...
with a rate.
This header file executes the tip dynamics (GPS dynamics of individual tips).
...
if(strcmp(Boundary_type,"repulsive")==0 || strcmp(Boundary_type,"static")==0){
  if(fabs(xt)>SizeX/2.0){
    if(strcmp(Boundary_type,"static")==0){state[i]=0;v[i]=0;}
    if(strcmp(Boundary_type,"repulsive")==0){state[i]=-1;v[i]=-Vs;}
  }
   if(fabs(yt)>SizeY/2.0){
    if(strcmp(Boundary_type,"static")==0){state[i]=0;v[i]=0;}
    if(strcmp(Boundary_type,"repulsive")==0){state[i]=-1;v[i]=-Vs;}
  }
}
...
void Tip_Dynamics(){
  ...
  for (int i=1; i<=n_tip; i++){
    if (Tiptype[i]==1){///if the tip is free then do the dynamics
      if(Tipl[i]>0){///three state tip dynamics
        if(state[i]==1){////Growing phase dynamics
          ...
          if(ran2(&idum)<pt){
            if (ran2(&idum)<r1){///G->S transition
              state[i]=-1;
              v[i]=-Vs;//assign shrinking velocity
            }else{
              state[i]=0;//G->P transition
              v[i]=0.0;//assign paused velocity
            }
          }
